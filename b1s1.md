...
Para mi punto de vista la que tiene mayor facilidad es la de jshell
...
print(get_average([1, 2, 3, 4, 5]))  # salida: 3.0
```
solucion en Scala 3
```Scala 3
...
@main def run() =
  println(getAverage(List(1, 2, 3, 4, 5)))
  ```
  En los tres lenguajes (Haskell, Python y Scala 3), el código funcional elimina el uso de variables mutables y bucles explícitos, reemplazándolos por funciones de orden superior como sum, length, y size.
Este estilo declarativo es más conciso y fácil de mantener. Entre ellos, Python y Scala 3 ofrecen una sintaxis más sencilla y legible, mientras que Haskell representa el paradigma funcional en su forma más pura.
